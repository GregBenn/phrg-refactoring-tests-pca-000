...
RSpec.describe Post, type: :model do
  ...
  it "is invalid with too short content" do
    expect(build(:post, content: "Short Circuit")).to be_invalid
  end
  ...
  it "is invalid with a category outside the choices" do
    expect(build(:post, category: "Documentary")).to be_invalid
  end

  it "is invalid if not clickbait" do
    expect(build(:post, title: "This is not clickbait")).to be_invalid
  end
